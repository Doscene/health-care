...
class RecordRepository @Inject constructor(
    ...
    suspend fun addBpRecord(systolic: Int, diastolic: Int, heartRate: Int? = null): Result<BpRecordDto> {
        return try {
            val resp = api.addBpRecord(AddBpRecordRequest(systolic, diastolic, heartRate))
    ...
    suspend fun addBgRecord(type: String, value: Double): Result<BgRecordDto> {
        return try {
            val resp = api.addBgRecord(AddBgRecordRequest(type, value))
            if ((resp.code == 0 || resp.code == 200) && resp.data != null) {
                Result.success(resp.data)
            } else {
                Result.failure(Exception(resp.message))
